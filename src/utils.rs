...
pub(crate) fn hex_dump(buf: &[u8]) -> String {
    buf.iter().fold(String::with_capacity(2 * buf.len()),
...
pub(crate) fn scan_sni_authorization<'a>(sni: &'a str, hostname: &str) -> Option<authorization::Source<'a>> {
    sni.strip_suffix(hostname)
        .and_then(|seek| seek.strip_suffix('.'))
        .map(|x| authorization::Source::Sni(x.into()))
}
